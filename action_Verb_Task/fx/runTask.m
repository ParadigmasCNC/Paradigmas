...
for i = 1:log.numTrials
    ...
    if i == 1 %% for instructions to display
        % Save in log
        [counter, log] = sendTrigger(log, NaN, NaN, NaN, NaN, log.events.taskStart, 'TaskStart', toc, counter);
        [counter, log] = sendTrigger(log, NaN, NaN, NaN, NaN, log.events.instructionsStart, 'InstructionsTaskStart', toc, counter);
        
        % Task instructions if training done before
        if log.training
            % Draw instructions
            Instructions = DrawFormattedText(ptb.w.pointer,log.texts.startTaskTxt{1,1},'center','center',ptb.fontColor);
            ...
            % Draw instructions
            Instructions = DrawFormattedText(ptb.w.pointer,log.texts.startTaskTxtNT{1,1},'center','center',ptb.fontColor);
            Screen('Flip',ptb.w.pointer, Instructions)
            WaitSecs(1);
        ...
        [counter, log] = sendTrigger(log, NaN, NaN, NaN, NaN, log.events.firstBlockStart, 'FirstBlockStart', toc, counter);
    end
    
    % Halfway break
    if i == round(log.numTrials/2) %% for a break at a particular trial (halfway currently)
        % Save in log
        [counter, log] = sendTrigger(log, NaN, NaN, NaN, NaN, log.events.firstBlockEnd, 'FirstBlockEnd', toc, counter);
        [counter, log] = sendTrigger(log, NaN, NaN, NaN, NaN, log.events.breakStart, 'BreakStart', toc, counter);
        
        % Half way break txt
        Welcome = DrawFormattedText(ptb.w.pointer,log.texts.breakTxt{1,1},'center','center',ptb.fontColor);
        Screen('Flip',ptb.w.pointer, Welcome)
        ...
        % Save in log
        [counter, log] = sendTrigger(log, NaN, NaN, NaN, NaN, log.events.breakEnd, 'BreakEnd', toc, counter);
        [counter, log] = sendTrigger(log, NaN, NaN, NaN, NaN, log.events.secondBlockStart, 'SecondBlockStart', toc, counter);
    end
    ...
    [counter, log] = sendTrigger(log, i, NaN, NaN, NaN, log.events.wordStart, log.wordList{i,2}, toc, counter);
    
    % display word
    Instructions = DrawFormattedText(ptb.w.pointer,log.wordList{i,1},'center','center',ptb.fontColor);
    Screen('Flip',ptb.w.pointer, Instructions)
    ...
    while (toc(extra.t0resp) < log.data.respDur)
        ...
        if keyIsDown
            ...
            if strcmp(cckey,'LeftArrow')
                ...
                if log.wordList{i,3} == 1; extra.correct = 1; else; extra.correct = 0; end
                break
            elseif strcmp(cckey,'RightArrow')
                respCode = 'right';
                extra.respTime(i) = toc(extra.t0resp) - ptb.scrn.Slack;
                if log.wordList{i,3} == 0; extra.correct = 1; else; extra.correct = 0; end
                break
            end
        end
    end
    ...
    % Save information
    [counter, log] = sendTrigger(log, i, respCode, extra.correct, extra.respTime(i), log.events.wordEnd, [log.wordList{i,2},'_Answer'], toc, counter);
