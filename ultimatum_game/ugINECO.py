...
TASK_REPETITIONS = 10      # Number of games with other people. Eran 10
NumberOfTrials=20 			#Number of trials in each task. Eran 20
...
PARALLEL_ADDRESS = 0xC020

###Video
file_name_video = "D:/EXTASIS/11.Ultimatum Game/UG_python-4_Andando/web_streaming.mpg"
window_name = "window"
...
# estaba todo comentado el screenconecting
...
#  def playMovie() estaba descomentado
def playMovie():
    FPS = 60
    clock = pygame.time.Clock()
    movie = pygame.movie.Movie("D:/EXTASIS/11.Ultimatum Game/UG_python-4_Andando/web_streaming.mpg")
...
#    #screen.fill(BLACK)
#    #pygame.display.update()
#    vidFile=cv2.VideoCapture(file_name_video)
...
try:
    ...
    #EEG
    if EEG:
        parallel.setPortAddress(PARALLEL_ADDRESS)
        parallel.setData(0)
    ...
    if PRACTICE:
        ...
        # Actual game
        opponents = range(TASK_REPETITIONS)
        ...
        #screen = pygame.display.set_mode((0,0), pygame.FULLSCREEN)
        #screenStartRound("En 3 segundos comenzara el juego")
        # Agregado
        #win = visual.Window(size=(1024, 768), pos=None, color=('black'), colorSpace='rgb', allowGUI=True ,fullscr=True, monitor='testMonitor', units='height',winType='pyglet', useFBO=True)
        #Video = visual.MovieStim3(win, filename=file_name_video, size=(1024, 768), flipVert=False, noAudio=True)
        ...
        for i in range(TASK_REPETITIONS):
            ...
            sendMark(30)
            proc(gameState, NumberOfTrials, "Juego con otra persona")
