function mainTextos()
    ...
    disp(['Elegir para el grupo ',possibleGroups{log.Group}, ' la permutacion: ',num2str(NOTOCAR)]);
    
    % Change it for save
    if NOTOCAR == 4
        NOTOCAR = 1;%#ok
    else
        NOTOCAR = NOTOCAR + 1;%#ok
    end
    ...
    for introIter = 1:length(introduction)
        ...
        [~,~,~] = DrawFormattedText(hd.window, introduction{introIter}, 'center', 'center');
        ...
        if EEG
            ...
            for eventIter = 1:length(log.events.(['eventList_',num2str(tasksIter)]))
                while toc < log.events.(['latencyList_',num2str(tasksIter)])(eventIter)/1000 - 0.002
                    % Keep checking when toc is higher than the mark and mark
                    % it has a 0.001 ms error rate for which we deduce it from
                    % the timer
                end
